...
```mermaid
flowchart LR
    subgraph before["❌ ANTES DE MCP"]
        A1["🤖 Agente 1"] --> T1["🔧 Tools hardcodeadas<br/>(duplicación)"]
        A2["🤖 Agente 2"] --> T2["🔧 Las mismas tools<br/>(otra vez)"]
        A3["🤖 Agente 3"] --> T3["🔧 Implementación diferente<br/>(bugs)"]
    end
...
**✅ CON MCP** - Beneficios: Un servidor, múltiples clientes, mantenimiento centralizado

```mermaid
flowchart LR
    subgraph after["✅ CON MCP"]
        A1["🤖 Agente 1"]
        A2["🤖 Agente 2"]
        A3["🤖 Agente 3"]
        C["📡 MCP Client<br/>(Protocolo)"]
        S["🔌 MCP Server<br/>(Una vez)"]
...
```mermaid
flowchart TB
    subgraph arch["🏗️ ARQUITECTURA MCP"]
        ...
        subgraph client["MCP Client"]
            C["📡 SDK Client<br/>(Protocolo JSON-RPC)"]
        end
...
sequenceDiagram
    ...
    rect rgb(243, 229, 245)
        Note over C,L: 2. Decisión del LLM
        L->>C: Quiero usar "search"
    end

    rect rgb(255, 243, 224)
        Note over C,S: 3. Ejecución
