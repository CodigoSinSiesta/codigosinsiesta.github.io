...
```mermaid
flowchart TB
    subgraph flow["🔧 FLUJO DE TOOL CALLING"]
        A["👤 Usuario envía mensaje<br/>'¿Cuál es el clima en Madrid?'"]
        B["🧠 LLM analiza y decide usar herramienta<br/>tool: 'get_weather', input: { city: 'Madrid' }"]
        C["⚙️ Tu código ejecuta la herramienta<br/>weatherAPI.get('Madrid')"]
        D["📤 Envías resultado al LLM<br/>{ temperature: 22, condition: 'sunny' }"]
        E["💬 LLM genera respuesta final<br/>'El clima en Madrid es soleado con 22°C'"]
...
```mermaid
flowchart TB
    subgraph react["🔄 CICLO ReAct"]
        T["💭 THOUGHT<br/>(Razonar)<br/>'Necesito buscar información<br/>sobre TypeScript...'"]
        A["⚡ ACTION<br/>(Actuar)<br/>search_web({ query: 'TypeScript' })"]
        O["👁️ OBSERVATION<br/>(Observar)<br/>[{ title: 'TS Handbook', ... }]"]
        Q{"¿Listo?"}
        F["✅ Respuesta Final"]
    end
...
## 🛠️ Proyecto Práctico: Agente Simple de Tareas
...
async function main() {
  ...
  console.log('🗂️  Agente de Tareas - Demo');
  console.log('═'.repeat(50));
